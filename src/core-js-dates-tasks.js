...
function getDayName(date) {
  ...
  return parseDate.toLocaleDateString('en-US', { weekday: 'long' });
}
...
function formatDate(date) {
  ...
  const hours = dateObject.getUTCHours() % 12 || 12;
...
function getWeekNumberByDate(date) {
  ...
  const diff = Math.round((date - firstThursday) / (1000 * 60 * 60 * 24));
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      schedule.push(
        `${String(currentDate.getDate()).padStart(2, '0')}-${String(currentDate.getMonth() + 1).padStart(2, '0')}-${String(currentDate.getFullYear())}`
      );
